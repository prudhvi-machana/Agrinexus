...
# Route to manage crops based on aadhar_id
...
# Route to search crops owned by a farmer
...
# Route to add a new crop for a farmer
...
def add_crop(aadhar_id):
    ...
    harvest_date = request.form['harvest_date']  # Optional, can be empty
    soil_ph = request.form['soil_ph']
    crop_suggestion = request.form.get('crop_suggestion', '')  # Use get to avoid KeyError

    try:
        # Validate start_date and end_date
        if not harvest_date:  # If end_date is empty, we can set it to None
            harvest_date = None
        else:
            # Ensure end date is later than start date if provided
            if datetime.strptime(harvest_date, "%Y-%m-%d") <= datetime.strptime(planting_date, "%Y-%m-%d"):
                flash("End date must be later than the Start date.", 'error')
                return redirect(url_for('manage_crops', aadhar_id=aadhar_id))
        
        # Insert crop into database
        cursor.execute(""" 
            INSERT INTO crops (aadhar_id, land_id, crop_name, crop_size, planting_date, harvest_date, soil_ph, crop_suggestion)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """, (aadhar_id, land_id, crop_name, crop_size, planting_date, harvest_date, soil_ph, crop_suggestion))
        ...
        mysql.connection.rollback()
        flash(f"Error adding crop: {err}", 'error')
...
# Route to update crop information
@app.route('/update_crop/<aadhar_id>/<crop_id>', methods=['POST'])
def update_crop(aadhar_id, crop_id):
    if 'auth_email' not in session:
        flash('You need to log in to access this page.', 'error')
        return redirect(url_for('auth_login'))
    soil_ph = request.form['soil_ph']
    crop_suggestion = request.form['crop_suggestion']
    ...
    try:
        cursor.execute(""" 
            UPDATE crops
            SET harvest_date=%s soil_ph = %s ,crop_suggestion = %s
            WHERE aadhar_id = %s AND crop_id = %s
        """, (soil_ph, crop_suggestion, aadhar_id, crop_id))
        mysql.connection.commit()
        flash("crop information updated successfully!", 'success')
    except Exception as e:
        mysql.connection.rollback()  # Rollback in case of error
        flash(f'Error updating crop information: {e}', 'error')
    finally:
        cursor.close()
    return redirect(url_for('manage_crops', aadhar_id=aadhar_id))

# Route to delete a crop
@app.route('/delete_crop/<aadhar_id>/<crop_id>', methods=['POST'])
def delete_crop(aadhar_id, crop_id):
    if 'auth_email' not in session:
        flash('You need to log in to access this page.', 'error')
        return redirect(url_for('auth_login'))
    ...
    cursor = mysql.connection.cursor()
    try:
        # Soft delete: Update the 'deleted' column to 1
        cursor.execute("""
            UPDATE crops
            SET deleted = 1
            WHERE aadhar_id = %s AND crop_id = %s
