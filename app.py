...
# Twilio credentials
account_sid = 'ACd6bb71111b68555dfe7fc4a07077e5ab'
auth_token = 'b5d92db6f0771911a3469ea95b9eaa17'
...
def farmer_login():
    if request.method == 'POST':
        ...
        # Query the database to find the farmer's details
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute("SELECT * FROM farmers WHERE aadhar_id = %s AND phone_no = %s", (aadhar_id, phone_no))
        farmer = cursor.fetchone()
        cursor.close()
        ...
        if farmer:
            ...
            if farmer['first_login']:  # If first login
                # Send the welcome message using Twilio
                message = client.messages.create(
                    body="Welcome to AgriData! We’re glad to have you on board.",
                    from_='+14402615390',  # Your Twilio number
                    to=f'+91{phone_no}'
                )
                print("Welcome message sent with SID:", message.sid)

                # Update the farmer's record to mark the first login as false
            ...
            flash('Login successful!', 'success')
            return redirect(url_for('farmer_details'))  # Redirect to the farmer details page
        else:
            flash('Invalid Aadhar ID or Phone Number.', 'error')
...
def farmer_login():
    if request.method == 'POST':
        ...
        if farmer:
            # Farmer found, set session variables
            session['aadhar_id'] = farmer['aadhar_id']
            session['farmer_name'] = farmer['farmer_name']

            # Check if it's the first login
            if farmer['first_login']:  # If first login
                # Log the notification into farmer_notifications table
                cursor.execute("""
                    INSERT INTO farmer_notifications (farmer_id, notification_type, notification_message, sent)
                    VALUES (%s, %s, %s, %s)
                """, (farmer['farmer_id'], 'Welcome Message', 'Send a welcome message to the new farmer', False))
                mysql.connection.commit()

                # Send the welcome message using Twilio
                message = client.messages.create(
                    body="Welcome to AgriNexus! We’re glad to have you on board.",
                    from_='+14402615390',  # Your Twilio number
                    to=f'+91{phone_no}'    # Farmer's phone number
                )
                print("Welcome message sent with SID:", message.sid)

                # After sending the message, update the notification status
                cursor.execute("""
                ...
                print(f"Updated first login status for farmer {aadhar_id}.")

            # Login successful, redirect to the farmer details page
            flash('Login successful!', 'success')
            return redirect(url_for('farmer_details'))  
        else:
            flash('Invalid Aadhar ID or Phone Number.', 'error')
